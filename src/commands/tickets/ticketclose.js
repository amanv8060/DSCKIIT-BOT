...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const user = message.guild.members.cache.find(user=>user.id === message.channel.name.substring(7));
    ...
    // .then(function (messages)  {
      ...
      const row = db.prepare("SELECT * from servers WHERE guildid = ?").get(message.channel.guild.id);
     ...
     user.send({
      ...
      const logchannel = message.guild.channels.cache.find(channel => channel.id === row.logChannelId);
